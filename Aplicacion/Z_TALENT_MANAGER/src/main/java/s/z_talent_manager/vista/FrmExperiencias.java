...
public class FrmExperiencias extends JPanel {
 ...
 try {
    ...
    // Se obtiene la experiencia de la lista usando el índice de la tabla
    Experiencia expAEliminar = c.getExperiencias().get(filaSeleccionada);
    
    // Se borra de la lista del objeto Candidato
    c.getExperiencias().remove(expAEliminar);
    
    // Se asgina el resultado a la variable 'actualizado'

    Candidato actualizado = ZTalentManagerServicio.getServicio().modificarCandidato(c);
    
    // 'actualizado' para refrescar la sesión y la tabla
